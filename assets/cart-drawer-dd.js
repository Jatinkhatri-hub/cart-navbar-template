...
//   }
  ...
  document.querySelectorAll('.info-circle').forEach(infoCircle => {
    infoCircle.addEventListener('click', (e) => {
      ...
      // Show the dropdown when the info circle is clicked
      toggleSubscriptionDropdown(lineIndex, true);
    });
  });

  // Add event listeners to update checkbox and dropdown status when the cart is updated
  function updateSubscriptionStatus() {
    document.querySelectorAll('.selling-plan-selector').forEach(selector => {
      ...
      // Automatically check or uncheck the subscription checkbox based on whether a subscription is selected
      checkbox.checked = hasSubscription;

      // Show or hide the dropdown based on subscription
      toggleSubscriptionDropdown(lineIndex, hasSubscription);
    });
  }

  // Call updateSubscriptionStatus() on page load or when the cart drawer is updated
  updateSubscriptionStatus();

  // Event listener to refresh subscription status after adding a product to the cart
  document.querySelectorAll('form[action="/cart/add"]').forEach(form => {
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      ...
      // Update cart drawer and refresh subscription status
      await updateCartDrawer();
      updateSubscriptionStatus();
    });
  });

  // Function to update cart drawer dynamically after AJAX request
  async function updateCartDrawer() {
    const res = await fetch("/?section_id=cart-drawer-new");
    const text = await res.text();
    ...
    // Reapply event listeners to the new elements
    addCartDrawerListeners();
    console.log(html);
  }

  // Function to reapply event listeners after cart drawer update
  function addCartDrawerListeners() {
    // Reapply subscription logic after drawer update
    document.querySelectorAll('.subscription-checkbox').forEach(checkbox => {
      checkbox.addEventListener('change', (e) => {
        const lineIndex = e.target.getAttribute('data-line');
        const isChecked = e.target.checked;
    ...
    // Call updateSubscriptionStatus again after any changes
    updateSubscriptionStatus();
  }
